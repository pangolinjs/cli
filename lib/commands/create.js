...
async function create (name) {
  ...
  console.log('\n' + chalk.hex('#ff721f').bold(`Pangolin CLI v${version}`) + '\n')
  ...
  try {
    ...
    const answers = await inquirer.prompt([{
      type: 'confirm',
      name: 'continue',
      message: chalk`The directory {blue ${dir}} already exists.\n  Continue?`,
      default: false
    }])
  ...
  // Write package.json
  const packageName = path.basename(name) === '.' ? null : path.basename(name)
